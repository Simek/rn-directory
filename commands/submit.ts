...
import type {LibraryDataEntryType, PackageJsonRepository} from "../types";
...
export default async function submit() {
  ...
  if (directoryData[packageName]) {
    console.warn(`The package already exist in the directory. Visit the package page at https://reactnative.directory/package/${packageName}`)
    process.exit(0);
  }
  ...
  if (!repositoryData) {
    console.error('You need to define the repository data inside `package.json` file, see: https://docs.npmjs.com/cli/v11/configuring-npm/package-json#repository')
    process.exit(1);
  }
  ...
  if (!librariesArray.find(({ githubUrl }) => githubUrl === repositoryUrl)) {
    // TODO: cleanup entry
    librariesArray.push({
      githubUrl: repositoryUrl,
      ...(directoryExist('example') ? {
        examples: [
          `${repositoryUrl}/tree/HEAD/example`
        ]
      } : {}),
  ...
  await Bun.write('commit.json', JSON.stringify({
      message,
      branch: branchName,
